...
{
    public class NewInnovationController : BaseController
    {
        ...
        public NewInnovationController(IMasterRepository master, IInnovationRepository innovationRep, Microsoft.AspNetCore.Hosting.IHostingEnvironment env)
        {
            this.master = master;
        ...
        public IActionResult GetSubmittedData()
        {
            var empId = HttpContext.Session.GetString("UserName");
            var result = innovationRep.GetInnovation(empId);
            return Ok(result);
        }
        public IActionResult GetPendingData()
        {
            var empId = HttpContext.Session.GetString("UserName");
            var result = innovationRep.GetPendingInnovation(empId);
            return Ok(result);
        }
        [HttpPost]
        ...
        {
            if (innovation.PlatformTypeId != "16")
            {
        ...
        [HttpPost]
        public IActionResult UpdateInnovationData([FromForm]InnovationStats innovationStats)
        {
            var uploadPath = Path.Combine(hostingEnv.WebRootPath, "ImageUpload");
            ...
            {
                var postedFile = innovationStats.PostedFile;
            ...
            var result = innovationRep.UpdateInnovationDetails(innovationStats, file_name);
