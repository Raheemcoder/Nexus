...
namespace Ideation.Controllers
{
    ...
    public class EffortTrackerController : BaseController
    {
        IEffortRepository effortRepository;
        IApproveRepository approveRepository;
        IApportionRepository apportionRepository;
        public EffortTrackerController(IEffortRepository effortRepository, IApportionRepository apportionRepository, IApproveRepository approveRepository) : base()
        {
        ...
        public IActionResult EffortTracker(string StartDate, string EndDate)
        {
            var UserId = HttpContext.Session.GetString("UserName") ?? string.Empty;
            EffortTracker effort = new EffortTracker();
            DateTime now = DateTime.Now; // get the current date and time
            //DateTime now = new DateTime(2023,2,5);
            CultureInfo culture = CultureInfo.CurrentCulture; // get the current culture
            DayOfWeek firstDayOfWeek = culture.DateTimeFormat.FirstDayOfWeek; // get the first day of the week in the current culture
            //DateTime firstDay = now.AddDays(-(int)now.DayOfWeek + (int)firstDayOfWeek);
            DateTime firstDay = now.AddDays(-(int)now.DayOfWeek);
            DateTime lastDay = now.AddDays(-(int)now.DayOfWeek + 5 + (int)firstDayOfWeek); // calculate the last day of the current week
            ...
            //effort.weekpicker = string.Concat(firstDay.ToString("dd/MM/yyyy").Replace("-", "/"), " - ", lastDay.ToString("dd/MM/yyyy").Replace("-", "/"));
        ...
        public IActionResult ProjectResourceMaster()
        {
            ...
            //ET.ResourceList = result.Select(p => new SelectListItem { Text= p.ResourceName,Value = p.ResourceName });
            ET.ResourceList = result.Select(p => p.ResourceName).Distinct().Select(p => new SelectListItem { Text = p, Value = p, Selected = true });
            //ET.ProjectDataList= result.Select(p => new SelectListItem { Text = p.ProjectCode, Value = p.ProjectCode });
            //       ET.ProjectDataList = result.Select(p => p.ProjectCode).Distinct().Select(p => new SelectListItem { Text = p, Value = p });
        ...
        //public JsonResult ApportionEffortsForApprovepage(String UserId,String EffortsDate,String ProjectId)
        //{
        //    CultureInfo culture = CultureInfo.CurrentCulture;
        //    DateTime Effortdate = Convert.ToDateTime(EffortsDate, culture);
        //    var result =approveRepository.GetTaskAndEfforts(UserId, Effortdate, ProjectId);
        ...
        public async Task<ActionResult> Index([FromServices] PowerBISettings powerBISettings)
        {
            try
            {
