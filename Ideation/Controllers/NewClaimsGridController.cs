...
{
    public class NewClaimsGridController : BaseController
    {
        private readonly IConfiguration? Configuration;
        private readonly IWebHostEnvironment webHostEnvironment;
        IClaimsGridRepository claimsRepository;
        public NewClaimsGridController(IClaimsGridRepository claimsRepository, IWebHostEnvironment webHostEnvironment, IConfiguration Configuration) : base()
        {
        ...
        public IActionResult ClaimsGridDocument()
        {
            ...
            claims.ClaimsGridList = GridList.ClaimsGridList;
            ...
            // claims.DivisionData = claimsRepository.GetDivision();
            return View(claims);
        }
        ...
        public IActionResult AddGridDocument()
        {
            ...
            return View("AddGridDocument", claims);
        }
        ...
        public IActionResult EditGridDocument(string ProjectNumber, string StatusId, string IsEdit, string GridId)
        {
            ...
            return View("EditGridDocument", claims);
        }
        ...
        public PartialViewResult GenerateClaimsPdfForAdd(ClaimsPdf generatePdfData)
        {
            var User = HttpContext.Session.GetString("UserName") ?? string.Empty;

            return PartialView(generatePdfData);
        }
        public PartialViewResult GenerateClaimsPdfForEdit(ClaimsPdf generatePdfData)
        {
            var User = HttpContext.Session.GetString("UserName") ?? string.Empty;
            
            if (generatePdfData.GridId != null)
            {
                var pdfdata = claimsRepository.GetClaimsApprovalData_PDF(generatePdfData.GridId);
                generatePdfData.DSGTeamApprovalData = pdfdata.DSGTeamApprovalData;
                generatePdfData.CFTTeamApprovalData = pdfdata.CFTTeamApprovalData;
                generatePdfData.DSGManagerApprovalData = pdfdata.DSGManagerApprovalData;
                generatePdfData.DSGSignOffApprovalData = pdfdata.DSGSignOffApprovalData;
                generatePdfData.AddendumRemarks = pdfdata.AddendumRemarks;
            }
        ...
        public FileResult GeneratePdfForAdd(string Type, [FromServices] IWebHostEnvironment env)
        {
            try
            {
        ...
        public ActionResult ExportToExcel(string projectNumber, string fileName, string GridId)
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                foreach (var item in ExcelData)
                {
                    ...
                    worksheet.Cells[currentRow, 6].Value = ProcessHTMLListsForExcel(item.Remarks);
                ...
                foreach (var item in ExcelData1)
                {
                    ...
                    worksheet2.Cells[currentRow2, 6].Value = ProcessHTMLListsForExcel(item.Remarks);
        ...
        public JsonResult GeneratePdfforSendmail(string toMailids, string remarks, string GridId, [FromServices] IWebHostEnvironment env, string FromButton)
        {
                     ...
                     "</tr>" +
                      "<tr style=\"text-align:center;\">" +
                           "<td style=\"border: 1px solid black;\">Claims grid signed off with review comments from all HUBS on</td>" +
                            "<td style=\"border: 1px solid black;\">" + projectData.DSGSignedOffDate + "</td>" +
                     "</tr></table>";
            }
            else
            {
                     ...
                     "</tr>" +
                      "<tr style=\"text-align:center;\">" +
                           "<td style=\"border: 1px solid black;\">Claims grid Addendum signed off on</td>" +
                            "<td style=\"border: 1px solid black;\">" + projectData.AddendumDate + "</td>" +
                      "</tr></table>";
            ...
            var result = SendMail.SendEmailLocal(subject, toMailids, ccMailIds, tableHtmlbody, stream, fileName);
            var JsonResult = Json("0");
        ...
        {
            string controllerName = "NewClaimsGrid";
