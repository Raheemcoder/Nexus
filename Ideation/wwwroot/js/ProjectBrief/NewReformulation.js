...
$('#BusinessInformationAdd').click(function () {
    ...
    if (isvalid) {
        ...
        $("input[type=radio][name=survey]:checked").val().toLowerCase() == "yes" ? $("#PackagingProfileDiv").show() : $("#PackagingProfileDiv").hide();
...
colmodels = [
    ...
    },
    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            ...
            return `<div class="action_icons">` +
                (rowobject.ImagesUploadHide == undefined || rowobject.ImagesUploadHide === "" ? `<i></i>` : `<a data-attr="` + options.rowId + `_download" onclick="DownloadPackageImage(` + options.rowId + `)" class="icon_color btn_button" title="Edit" id="` + options.rowId + `DownloadPackageImagesUpload"><i class="fas fa-download color-download mr-2" title="Download"></i></a>` +
                    '<a data-attr="' + options.rowId + 'view" class="icon_color btn_button" title="View" id="' + options.rowId + '" href="' + ROOT + 'NPDImages/' + rowobject.ImagesUploadHide + '" target="_blank" ><i class="fas fa-eye color-eye mr-2" title="View"></i></a><a data-attr="' + options.rowId + 'delete" onclick="DeletePackageImage(' + options.rowId + ')" class="icon_color btn_button" title="Delete" id="' + options.rowId + '"><i class="fas fa-trash color-delete mr-2" title="Delete"></i></a>') +
                `</div> `;
        }
    },
    {
...
$("#preview_expected").jqGrid({
    ...
    loadonce: true,
    viewrecords: true,
    pager: '#preview_pager_expected',
    rowNum: 20,
    scroll: 1,

    gridComplete: function () {
        var objRows = $("#preview_expected tbody tr");
        var objHeader = $("#preview_expected tbody tr td");
        ...
        jQuery("#preview_expected").jqGrid('hideCol', "Action");
    }
});
...
function validatesubmitform() {
    ...
    projectheaders[0].Division == "" ?
        (flag = false, $('#Error_Reformulation_Division').show(), document.getElementById('Reformulation_Table').scrollIntoView({ behavior: 'smooth' })) :
        $('#Error_Reformulation_Division').hide();

    projectheaders[0].Category == "" ?
        (flag = false, $('#Error_Reformulation_Category').show(), document.getElementById('Reformulation_Table').scrollIntoView({ behavior: 'smooth' })) :
        $('#Error_Reformulation_Division').hide();

    productdescription == "" ?
        ($("#err-prd_desc").show(), flag = false, document.getElementById('bh').scrollIntoView({ behavior: 'smooth' })) :
        $("#err-prd_desc").hide();

    reformulationBusinessInformation == "" ?
        ($("#err-business_info").show(), flag = false, document.getElementById('ship_location').scrollIntoView({ behavior: 'smooth' })) :
        $("#err-business_info").hide();

    $("#ProjectDetailsBusinessRational").val().trim() == "" ? ($('#Err-ProjectDetails-BusinessRational').show(), flag = false, document.getElementById('sd1').scrollIntoView({ behavior: 'smooth' })) :
        $('#Err-ProjectDetails-BusinessRational').hide();

    productdescription.length === 0 ?
        ($('#Err-ProductDescription').show(), flag = false, document.getElementById('bh').scrollIntoView({ behavior: 'smooth' })) :
        $('#Err-ProductDescription').hide();

    reformulationBusinessInformation.length === 0 ?
        ($('#err-business_info').show(), flag = false, document.getElementById('ship_location').scrollIntoView({ behavior: 'smooth' })) :
        $('#err-business_info').hide();

    sustainabilityGridData.length === 0 ?
        ($('#Error_Sustainability').show(), flag = false, document.getElementById('ship_address').scrollIntoView({ behavior: 'smooth' })) :
        $('#Error_Sustainability').hide();

    if ($("input[type=radio][name=survey]:checked").val().toLowerCase() == "yes")
    {
        reformulationPackagingProfileGrid.length === 0 ?
            ($("#err-expected").show(), flag = false, document.getElementById('PackagingProfileDiv').scrollIntoView({ behavior: 'smooth' })) :
            $("#err-expected").hide();
    }

    $('#ProjectName').val().trim() == "" ?
        ($("#Error-ProjectName").show(), flag = false, document.getElementById('ci').scrollIntoView({ behavior: 'smooth' })) :
...
function ValidateSaveForm() {
    ...
    projectheaders[0].Category == "" ?
        ...
        $('#Error_Reformulation_Division').hide();
    $('#ProjectName').val().trim() == "" ?
        (flag = false, $("#Error-ProjectName").show(), document.getElementById('ci').scrollIntoView({ behavior: 'smooth' })) :
        $("#Error-ProjectName").hide();
            ...
            if (isSaved) {
                ...
                if (deleteImagesList.length != 0) {
                    $.each(deleteImagesList, function (i, obj) {
                        $.ajax({
                            type: 'POST',
                            url: ROOT + "ProjectBrief/DeleteImageFile?fileName=" + obj,
                            async: false,
                            cache: false,
                            contentType: false,
                            processData: false,
...
$('#PackagingProfileAdd').click(function () {
    var isvalid = true;
    $('.Err-PPNoMoreSKU').hide();
    var primaryPackaging = CKEDITOR.instances["PackagingProfilePrimaryPackaging"].getData().trim();
                    ...
                    <th>
                        <span>
                            <div class="action_icons justify-center_1">
                                ...
                                <a class="btn-icon -delete"><i onclick="onDeletePackagingProfile(`+ ppRowId + `,'#PP_Table_` + ppRowId + `',0)" class="fas fa-trash color-delete" data-bs-toggle="modal" title="Delete"></i></a>
                        ...
                        <th>
                            <span>
                                <div class="action_icons justify-center_1">
                                    ...
                                    <a class="btn-icon -delete"><i onclick="onDeletePackagingProfile(`+ EditRowIdPackagingProfile + `,'#PP_Table_` + EditRowIdPackagingProfile + `',0)" class="fas fa-trash color-delete" data-bs-toggle="modal" title="Delete"></i></a>
...
function validatesubmitformToManager() {
    ...
    projectheaders[0].Division == "" ?
        (flag = false, $('#Error_Reformulation_Division').show(), document.getElementById('Reformulation_Table').scrollIntoView({ behavior: 'smooth' })) : $('#Error_Reformulation_Division').hide();
    projectheaders[0].Category == "" ?
        (flag = false, $('#Error_Reformulation_Category').show(), document.getElementById('Reformulation_Table').scrollIntoView({ behavior: 'smooth' })) : $('#Error_Reformulation_Division').hide();
    productdescription == "" ?
        ($("#err-prd_desc").show(), flag = false, document.getElementById('bh').scrollIntoView({ behavior: 'smooth' })) :
        $("#err-prd_desc").hide();
    reformulationBusinessInformation == "" ?
...
colmodels = [
    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            ...
            return '<div class="action_icons">' +
                '<span class="action-link"><a class="btn-icon -download DownloadImage" title="Download Image" onclick=DownloadUploadedImage("' + imageName + '") id="DownloadImage"><i class="fas fa-download color-download" aria-hidden="true"></i></a></span>' +
                '<span class="action-link"><a class="btn-icon -view ViewImage" onclick=ViewUploadedImage("' + imageName + '") target="_blank" id="ViewImage" title="View Image"><i class="fas fa-eye color-eye"></i></a></span>' +
                '<span class="action-link"><a class="btn-icon -delete hideBenchmark" title="Delete Image" onclick=DeleteBenchmarkImage("' + options.rowId + '")><i class="fas fa-trash color-delete"></i></a></span>' +
                '</div>';
        }
    },
    {
...
$("#Preview_Modal").click(function () {
    ...
    $(".PreviewBusinessRational").text($("#ProjectDetailsBusinessRational").val());
    $(".PreviewBenchmarksamples").text($("#ProjectDetailsBenchmarkSampleFormulation").val());
    $(".PreviewDesiredIndications").text($("#ProjectDetailsDesiredIndications").val());
    $(".PreviewDesiredDosageForm").text($("#ProjectDetailsDesiredDosageForm").val());

    $(".Previewtextbox").val($("#AdditionalRequirementsTextBox").val());
    $(".PreviewShelflife").text($("#AdditionalRequirmentsShelfLife").val());
    $(".PreviewFreefrom").text($("#AdditionalRequirmentsFreeFrom").val());
    $(".PreviewOthers").text($("#AdditionalRequirmentsOthers").val());
