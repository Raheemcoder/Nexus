var initialdata
var fromDateforrefresh = $("#fromdateforrefresh").val(); 
var toDateforrefresh = $("#todateforrefresh").val();
$(document).ready(function () {
	...
	CreateJQgrid(projectRemarksData)
});
...
colmodels = [
	{
		...
		formatter: function (cellvalue, options, rowobject) {
			if (rowobject.Task != null) {
				return '<div class="justify-center_"> <a href="#" id="Remarks_Icon" data-bs-toggle="modal"  data-createdby="' + rowobject.CreatedBy + '" data-task="' + rowobject.Task + '" data-createdon="' + rowobject.Date + '" onclick="Remarks_Icon(this)" data-bs-target="#remarks_section" title="Project Details" class= "btn-icon -edit " > <i class="fas fa-info-circle" > </i></a ></div>'
			}
			else {
...
function refreshJqGrid() {
	$("#ProjectRemarks_Grid").jqGrid('clearGridData');
	$("#ProjectRemarks_Grid").jqGrid('setGridParam', { data: initialdata }).trigger('reloadGrid');
	$("#Project_id").val('').trigger('change');
	$("#FromDate").val(fromDateforrefresh).trigger('change');
	$("#ToDate").val(toDateforrefresh).trigger('change');
...
$("#ExcelDownload").click(function () {
	var data = $('#ProjectRemarks_Grid').jqGrid('getGridParam', 'data');
	if (data.length == 0) {
		alert("No data in Grid");
	}
	else {
		debugger;
		$("#ProjectRemarks_Grid").jqGrid("exportToExcel", {
			includeLabels: true,
			includeGroupHeader: true,
			includeFooter: true,
			exportcol: false,
			fileName: "ProjectRemarks.xlsx",
			maxlength: 1000,
