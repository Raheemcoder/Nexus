...
function create_tr(projId, projName, division, state)
{
    ...
    `
        ...
        <tr class="valu" style="height:30px">
...
$("#searchbtn").on('click', function () {
    ...
    window.location.href = ROOT + 'EffortBooking/Apportion?q=' + Encrypt('StartDate=' + StartDate + '&EndDate=' + EndDate);
});
...
$(".SaveData").on('click', function () {
    ...
    $('#Task').val() == '' ? ($('#ApportionTaskErr').show(), flag = false, document.getElementById('Projectheader').scrollIntoView({ behavior: 'smooth' })) : $('#ApportionTaskErr').hide();
    if (rowCount > 0) {
        ...
        if (flag && isValidData && (errList.length == 0)) {
            ...
            confirm("Please note that once Saved, the apportioned efforts cannot be updated. Do you want to continue?", function () {
                ...
                isSubmitted = true;
                $.ajax({
                    url: ROOT + "EffortBooking/SaveApportionData",
                    type: "POST",
                    async: false,
                    dataType: "json",
                    data: {
                        EffortDetails: JSON.stringify(data),
                        Division: division,
...
function getDetails() {
    ...
    {
        $('#NoRecordsSelectedErr').show();
        document.getElementById('WeekEntry').scrollIntoView({ behavior: 'smooth' });
        return 0;
    }
}
...
$('#filterbtn').click(function () {
    ...
    if (flag) {
        ...
        $.ajax({
            type: "POST",
            url: ROOT + 'EffortBooking/GetProjectList',
            ...
            success: function (result) {
                ...
                if (result.length != 0) {
                    ...
                    $('#tfoot').append('<tr><td colspan="10" class="tf_td text-right"><b><label class="mt-2 viewbold">View 1 - ' + RecordsCount + ' of ' + RecordsCount + '</label></b></td></tr>');
                    leavesAndHolidaysHighlight();
                }
                else {
...
colmodels = [
    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            ...
            return '<div class="text-center action_icons">' +
                '<a onclick=deleteApportionData(' + rowobject.ApportionId + ',' + options.rowId + ') title="Delete" ><i class="fas fa-trash color-delete" aria-hidden="true"></i></a>' + '<a onclick = showApportionData(' + rowobject.ApportionId + ',' + options.rowId + ') title="View"><i class="fas fa-eye color-eye"></i></a>' +
                '</div>';
        }
    },
    {
...
$.ajax({
    type: "POST",
    url: ROOT + "EffortBooking/GetApportionViewData",
...
$('#apportionViewBtn').click(function () {
    ...
    var formattedMonthYear = moment($('#viewMonth').val()).format('M,YYYY');
...
function deleteApportionData(apportionID, rowId) {
    confirm("Are You Sure You Want To Delete The Apportion Data ?", function () {
        ...
        $.ajax({
            type: "POST",
            url: ROOT + "EffortBooking/DeleteApportionData",
...
function showApportionData(apportionID, rowId) {
    ...
    $.ajax({
        type: "POST",
        url: ROOT + "EffortBooking/ShowApportionData",
...
$(".btn-refresh").on('click', function () {
    ...
    $('#Classification option').prop("selected", true);
    $("#Classification").multiselect('refresh');
    $('#ProjectType option').prop("selected", true);
    $("#ProjectType").multiselect('refresh');
    $('#RnDTeam option').prop("selected", true);
    $("#RnDTeam").multiselect('refresh');
    createJqGrid();
});
