...
function GetSearchResult(formattedStartDate, formattedEndDate, searchText) {
    $.ajax({
        ...
        success: function (response) {
            ...
            //$('#prd_desc1').jqGrid('clearGridData');

            //if (responseData != undefined) {
            //    $("#prd_desc1").jqGrid('setGridParam', { data: responseData }).trigger("reloadGrid", [{ page: 1 }]);
            //}
...
add_color = function (rowId, val, rowobject) {
    ...
    else if (rowobject.IsRollback == 1 || rowobject.IsRollback == 2) {
...
function createGrid(data) {
    jQuery("#prd_desc1").jqGrid({
        ...
        colModel: [
            ...
            },
            {
                ...
                //                            <a href="'+ROOT+'RID/AddIngredient?ingredientid=rowobject.IngredientId'" class="btn-icon -edit "><i class="fas fa-edit"></i>Edit</a>
                //                            <a href="#" class="btn-icon -delete"><i class="fas fa-trash" data-bs-toggle="modal" data-bs-target="#DeleteModal" title="Delete"></i></a>
                //                        </div>`;
                //}
                formatter: function (cellvalue, options, rowobject) {
                    return '<div class="justify-center_">' +
                        '<a href="' + ROOT + 'RID/AddIngredient?q='+Encrypt('ingredientid=' + rowobject.IngredientId )+'" class="btn-icon -edit" title="Edit">' +
                        '<i class="fas fa-edit"></i>' +
                        '</a>' +
                        '<a href="#" class="btn-icon -delete" onclick="DeleteIngredient(this)" data-ingredientid="'+rowobject.IngredientId+'" title="Delete">' +
    ...
    $('#prd_desc1').closest('.ui-jqgrid-bdiv').css({ 'max-height': '55vh' });
