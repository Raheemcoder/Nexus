...
$("#savebtn").on("click", function () {
    ...
    $.each(region_array, function (i, obj1) {
        ...
        var regionData = gridData.filter(function (obj) { return (!(parseInt(obj.RegulatoryStatus) === 0 && (obj.CRemarks === "" || obj.CRemarks === null)) && parseInt(obj1) === parseInt(obj.RegionId)) });

        regulatoryvalidlength === totallength && remarksvalidlength === totallength ? totalvalidlength++ : totalvalidlength;


        if (isValid_region && isValid_remarks) {
    ...
    if (totalvalidlength === 0) {
        if (totalvalidlength_regulatory == 0 && totalvalidlength_remarks === 0) {
            
            alert("Please enter the Regulatory Status and Compliance Remarks for " + region_name + " Region ");
            isValid = false;
        }
        else if (totalvalidlength_regulatory === 0) {
            alert("Please enter the Regulatory Status for " + region_name + " Region ");
            isValid = false;
        }
        else if (totalvalidlength_remarks === 0) {
            alert("Please enter the Compliance Remarks for " + region_name + " Region ");
            isValid = false;
        }
