...
$('#BI_TotalBusinessValue').val(ReformulationJQgrid.ReformulationBusinessInformation.length > 0 ? ReformulationJQgrid.ReformulationBusinessInformation[0].TotalBusinessValue : "");
    ...
    $("#business_info").jqGrid({
        ...
        gridComplete: function () {
            ...
            if (statusId == '2' || statusId == '7' || statusId == '5' || statusId == '16' || statusId == '13') {
                ...
                // jQuery("#business_info").jqGrid('hideCol', "PropesedName");
            }
...
if (statusId == "2" || statusId == "13") {
    ...
    //if ($('#expected').jqGrid('getGridParam', 'data').length > 0) {
    //    $('.PackagingProfileHeadingName').show()
    //}
    //else {
    //    $('.PackagingProfileHeadingName').hide()
    //}
    $("#PackagingProfileGrid").show()
    if (statusId == "13") {
        $('#statusHeader').text("Reformulation - Brief Demoted to HGML");
    }
    else {
...
if (statusId == "7") {
    ...
    if (statusId == "8" || statusId == "9") {
        var textNode = document.createTextNode('Reformulation-' + statusName);
        ...
        var parentElement = document.getElementById('statusHeader');
        ...
        colmodels = [
            {
                ...
                formatter: function (cellvalue, options, rowobject) {
                    return `<div class="text-center icon_section align-items-left">
            <a onclick = onEditPmdData(` + options.rowId + `) class="icon_color btn_button" title="Edit" id="edit_worksheet"><i class="fa fa-edit mr-2" title="Edit"></i></a>
...
$('#ProductDescriptionAdd').click(function () {
    ...
    if (isvalid && noDuplicate) {
        ...
        //var productList = $("#prd_desc").jqGrid("getCol", "ExistingName");
...
$(document).ready(function () {
    ...
    if (ReformulationJQgrid.ReformulationProductDetails.length != 0) {
        ...
        //$("#PackagingProfileProduct,#BusinessInformationProductName,#PmdProductName,#HgmlData_ProductNameApprovePage,#HgmlData_ProductName").append(productOption);
        ...
        if (statusId == "5" || statusId == "16" || statusId == "6" || statusId == "12") {
            ...
            //targetCostProductNameList = $("#TargetCost_Grid").jqGrid("getCol", "Product");
            //targetCostProductNameList = $.grep(productDescriptionProductNameList, function (el) { return $.inArray(el, targetCostProductNameList) == -1 });
...
$('#BusinessInformationAdd').click(function () {
    ...
    //        isvalid = false;
    //    }
    //    else if (reformulationPackagingProfileGrid.length == reformulationBusinessInformation.length) {
    //        
    //        $("#Err-PackagingProfileBI").show()
    //        isvalid = false;
    //    }
...
//ProjectDetailsBenchMarkSampleImageDownload
...
if (BenchMarkSampleImage != "") {
    ...
    //prop("href", ROOT + "NewInitiation/DownloadImageFile?fileName=" + filename);
...
$('#ProjectDetailsBenchMarkSampleImageDownload').click(function () {
    ...
    if (ProjectDetailsBenchMarkSampleImage.length > 0) {
        $('#ProjectDetailsBenchMarkSampleImageDownload').prop("href", ROOT + "NewInitiation/DownloadImageFile?fileName=" + ProjectDetailsBenchMarkSampleImage);
        return true;
    }
});
    ...
    },
    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            ...
            if (statusId == '1' || statusId == '8' || statusId == '9') {
                return `<div class="text-center icon_section align-items-left">` +
                    (rowobject.PackagingProfileImage == undefined || rowobject.PackagingProfileImage == "" || rowobject.PackagingProfileImage == null ? `<i></i>` : `<a onclick="DownloadPackageImage(` + options.rowId + `)" class="icon_color btn_button" title="Download" id="` + options.rowId + `DownloadPackageImagesUpload"><i class="fa fa-download mr-2" title="Download"></i></a>` +
                        `<a class="icon_color btn_button" title="View" id="'` + options.rowId + '" href="' + ROOT + 'NPDImages/' + rowobject.PackagingProfileImage + `" target="_blank" ><i class="fa fa-eye mr-2" title="View"></i></a>` +
                        `<a onclick='onDeleteImage("expected","` + rowobject.PackagingProfileImage + `","` + options.rowId + `")' class="icon_color btn_button" title = "Delete" > <i class="fa fa-trash" title="Delete"></i></a >`) +
                    `</div> `;
            }
            else {
                return `<div class="text-center icon_section align-items-left">` +
...
//    const productList = $("#expected").jqGrid("getCol", "Product");

//    if (productName != "") {

//        productList.includes(productName) ? ($(".Error_PP_Product").show().text('This Product already consists the definition, Please select the different Product')) : "";
//    }
//});
...
if (statusId == "1") {
    ...
    // ReformulationJQgrid['ReformulationPackagingProfile'] == "" ? ($("#PackagingProfile").hide(), $("#PackagingProfileGrid").hide()) : ($("#PackagingProfile").show(), $("#PackagingProfileGrid").show())


}
//if (statusName == "HGML Review") {

//        $('#Page_Heading').text('Reformulation-HGML Review');
//        $(".NoEdit").attr('readonly', true);
//        $(".HubReview").hide();
//    $(".HideClass").hide();
//    $(".product_profile").show();
...
function validatesubmitform() {
    ...
    /* SKU = CKEDITOR.instances["editornf"].getData();*/
    //SKU = $('#SKUDetails').val();
    //SKU == "" ? ($('#Err-ProductDescription-sku').show(), flag = false, $(window).scrollTop($('.sku-field').position().top)) : $('#Err-ProductDescription-sku').hide();
    ...
    reformulationBusinessInformation.length === 0 ? ($('#err-business_info').show(), flag = false, $(window).scrollTop($('#err-business_info').position().top)) : $('#err-business_info').hide();

    sustainabilityGridData.length === 0 ? ($('#Error_Sustainability').show(), flag = false) : $('#Error_Sustainability').hide();

    if ($("input[type=radio][name=survey]:checked").val() == "yes") {
        reformulationPackagingProfileGrid.length === 0 ? ($("#err-expected").show(), flag = false, $(window).scrollTop($('#err-expected').position().top)) : $("#err-expected").hide();
    }
...
function ReformulationPmdSendBack() {
    ...
    //if (previousStage == statusList[1].StatusName) {
    //    $('div#SendbackModal').modal('show');
    //    $('#SendbackModal').find('h5').text('Send Back To ' + statusList[1].StatusName);

    //    $('#SendbackModal').find('.control-label').text(' Are you sure you want to Send Back the Project Brief to ' + statusList[1].StatusName + ' ?');
    //}
    //else {
    $('div#SendbackModal').modal('show');
    $('#SendbackModal').find('h5').text('Send Back Confirmation');
    $('#SendbackModal').find('.control-label').text(' Are you sure you want to Send Back the Project Brief?');
    //}
...
function ReformulationPmdApprove() {
    ...
    //else if (pmdTargetCostData.length == 0) {
    //    $('.Err-TCEmpty').show();
    //}
    else {
...
function ReformulationHgmlReject() {
    ...
    $("#Reject_Button").click(function () {
        ...
        var rejectRemarks = $('#ReformulationRejectRemarks').val();
...
function ReformulationHgmlSave() {
    ...
    var flag = true;
    //if ($("#HgmlDataSendToHubConfirmation").val() != "") {
    var hgmlData = [];
...
function ReformulationHgmlSendtoHub() {
    ...
    YesNo === "" ? ($('#Error_DoYouWantSentToHUB').show(), flag = false, $(window).scrollTop($('#Hgml_Data-Field').position().top)) : $('#Error_DoYouWantSentToHUB').hide();
...
function ReformulationHubToHgml() {
    ...
    if ($('#HubApproveSelect :selected').text() == "Yes" || $('#HubApproveSelect :selected').text() == "No") {
        ...
        $(window).scrollTop($('.headerTable').position().top);
...
$('#sendHubHgmlApproveAddbBtn').click(function () {
    ...
    // ret.HubName === $('#HgmlData_HubDropdownApprove').val() ? $('#HubDupErr').show() : $('#HubDupErr').hide(), isValid=false;
...
function validatePmdReviewSendMailForm() {
    ...
    //var sendMailData = JsonFormReformulationPmdReviewData["HubApproveConfirmationList"];
...
if ($('#ViewStatus').val() == 'View') {
    ...
    if (statusId == "8" || statusId == "11") {
        $('#saveReformulation,#ReformulationSubmit,#PackagingProfile').hide();
...
//if (($('#ViewStatus').val() == 'View') && (statusId == 3)) {
...
//add_grid_ hgmldatagrid hideHgmlGrid mt - 3 hubmarkets
//Added by Sachin
...
//---------------------------------- Packaging profile Table display------------------------------------------------------
        ...
        $.each(packagingProfileData_1, function (i, obj) {
                    ...
                    <th>
                        <span>
                           <div class="justify-center_1">
                                <a class="btn-icon -edit Edittable"><i onclick="onEditPackagingProfile(`+ i + `)" class="fas fa-edit " title="Edit"></i></a>
                                <a class="btn-icon -delete deletetable ppDeleteHide"><i onclick="onDeletePackagingProfile(`+ i + `,'#PP_Table_` + i + `',0)" class="fas fa-trash" data-bs-toggle="modal" title="Delete"></i></a>
                                       ${obj.PackagingProfileImage !== '' && obj.PackagingProfileImage !== null ? `
                                       <a class="btn-icon -info imagesinfo" title="Images info" onclick="ShowImages(`+ i + `)" id="` + i + `"><img src='../images/multiimages.png'/></a>` : ''}
                            </div>
                        </span>
                    </th>
                </tr>
            ...
            <tbody>
                <tr>
                    ...
                    <td style="width:25%;">
                    <span class="remarkss">
                        <b> Secondary Packaging: </b><span class="remarks_Show show_secondarypack list_icon" style="display:none"><i class="fas fa-list"></i></span></span><span>`+ obj.SecondaryPackaging + `</span>
                    </td>
                    <td>
                         <span class="remarkss">
            ...
            //var viewContainer = $(`#PP_Table_${i}`).find(".image_table .image_view");
            //console.log(viewContainer)
            //viewContainer.empty(); // Remove the existing img tag

            //var viewButton = $("<img>"); // Create a new img tag
            //viewButton.attr("src", ROOT + 'NPDImages/' + obj.PackagingProfileImage);
...
if ($('#ViewStatus').val() == 'View') {
    ...
    $('#saveReformulation,#ReformulationSubmit').hide();
        ...
        <table class="mt-2" id="PP_Table_`+ (ppRowId) + `" style="width:100%">
            <thead>
                <tr>
                    <th colspan="2">
                        <b>Product : </b>
                        <span>`+ PackagingProfile.Product + `</span>
                    </th>
                    <th style="width:25%">
                        <b>SKU : </b>
...
$('.formulation_table').on('click', '.show_primarypack, .show_secondarypack,.show_tertiarypack,.show_desiredpack,.show_benchmark,.show_others,.show_mold', function () {
    //$('.formulation_table').on('click', '.show_primarypack',function()
...
$(document).on("click", "#TotalBusinessValue", function () {
    ...
    $.ajax({
        ...
        url: ROOT + 'NewInitiation/GetTotalBusinessValue?projectId=' + projectId + '&ProjectType=' + 2,
        success: function (Result) {
...
function validateEditDataSave() {
    ...
    //    $('div#SaveModal').modal('show');
    //} else {
    //    reformulationEditSaveData(); // Call the function to save the data
    //}
    ...
    //function reformulationEditSaveData() {
    //  $('#ReformulationSavePopUp').prop("disabled", true);
    $("table#Reformulation_Table tbody tr").each(function (i) {
...
if (statusId == '9' && $('#ViewStatus').val() != 'View') {
    ...
    $('#statusHeader').text("Reformulation - Pending for Approval");
...
if (statusId == '9' && $('#ViewStatus').val() == 'View') {
    ...
    $('.forPendingStage').show();
    $('.forDrafrStage').hide();
    $('#ReformulationSubmitToManager').hide();
...
function validatesubmitformToManager() {
    ...
    //    flag = false;
    //    confirm("Manger is not Mapped to you, please contact Admin to map the manager");
    //}
    if (flag) {
        $('div#SubmitModal').modal('show');
        $("#ConformReformulation").click(function () {
...
//if (statusId != '3' && statusId != '13' && statusId != '4' && statusId!= '14') {
//    $('.FieldRemarks').attr('readonly',true);
//}
...
function validateSendToUnderExploration() {
    ...
    $("#ReformulationSendToUnderEX").click(function () {
        ...
        $("#PopUp_SendToExpRemarks").keyup(function () {
            $("#PopUp_SendToExpRemarks").val() == "" ? $(".errorUnderExp").show() : $(".errorUnderExp").hide();
        });
    });
}
if (statusId == "16") {
...
colmodels = [
    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            ...
            return '<div class="text-center icon_section align-items-left">' +
                '<span class="action-link"><a class="btn-icon -download DownloadImage" title="Download Image" onclick=DownloadUploadedImage("' + imageName + '") id="DownloadImage"><i class="fas fa-download" aria-hidden="true"></i></a></span>' +
                '<span class="action-link"><a class="btn-icon -view ViewImage" onclick=ViewUploadedImage("' + imageName + '") target="_blank" id="ViewImage" title="View Image"><i class="fas fa-eye"></i></a></span>' +
                '<span class="action-link"><a class="btn-icon -delete2 hideBenchmark" style="display:none" title="Delete Image" onclick=DeleteBenchmarkImage("' + imageName + '")><i class="fas fa-trash"></i></a></span>' +
                '</div>';
        }
    },
    {
