...
function GetL2ApprovalData() {
    ...
    $.ajax({
        type: "POST",
        url: ROOT + "NewProjectInitiation/GetL2ApprovalList",
        dataType: "json",
        data: { Status: status },
...
colmodels = [

    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            return '<div class="d-flex action_icons align-items-center l2budget_" title="">' +
                '<a href="' + ROOT + 'NewProjectInitiation/BaseLineBudgetApprovalPhase2?q=' + Encrypt('ProjectId=' + rowobject.ProjectId) + '" class="text-info color-blue" title="">' + cellvalue + '</a>' +
                '</div>';
        }
    },
    {
    ...
    },
    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            if (cellvalue != "" && cellvalue != undefined && cellvalue != null) {
                var Budget = parseInt(cellvalue).toLocaleString('en-IN', { maximumFractionDigits: 0 })
                return '<a href="#" class="task_status value_info "> <span class="text-warning">' + Budget + '</span></a>'
            }
            else {
                return "";
            }
...
$('.ui-jqgrid-bdiv').css({ 'max-height': 'calc(-220px + 100vh)' });
