...
function getProjectslist(search) {
    ...
    $.ajax({
        type: "POST",
        url: ROOT + "NewProjectInitiation/GetL2ApprovalPendingDataForProject",
        dataType: "json",
    ...
    },
    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            if (rowobject.BaselineApprovalStatus == "L2 Approval Pending" || rowobject.BaselineApprovalStatus == "L1 Approval Pending") {
                ...
                return '<a href="#" class="task_status value_info"> <span class="text-warning baslinepending">' + Budget + '</span></a>'
            }
            else {
                return ''
            }
    ...
    $("#list").jqGrid({
        ...
        gridComplete: function () {
            ...
            for (var i = 0; i < List.length; i++) {
                if (List[i].AdditionalBudgetApprovalStatus === "L2 Approved" || List[i].BaselineApprovalStatus === "L2 Approved") {
                    approvedCount++;
                }
                else if (List[i].AdditionalBudgetApprovalStatus === "L1 Approval Pending" || List[i].BaselineApprovalStatus === "L1 Approval Pending") {
                    pendingcount++
                }
            }

            if (approvedCount === List.length || pendingcount == List.length || $("#Role").val().toLowerCase() == "admin") {
                $("#cbox").hide();
                $("#ApproveData").hide();
