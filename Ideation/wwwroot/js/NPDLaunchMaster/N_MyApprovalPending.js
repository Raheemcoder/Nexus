...
findDivisionCount(myApprovalPendingData);
...

    {
        ...
        formatter: function (cellvalue, options, rowobject) {
            return '<div class="d-flex action_icons align-items-center">' +
                '<div><a class="edit-color" title="Approve" onclick=SingleApproveOrReject(' + rowobject.MaterialCode + ',"Yes")><i class="fas fa-thumbs-up"></i></a><a title="Reject" onclick=SingleApproveOrReject(' + rowobject.MaterialCode + ',"No") data-bs-toggle="modal" data-bs-target="#reject" ><i class="fas fa-thumbs-down color-eye"></i></a></div>' +
                '</div>';
        }
    },
...
function NPDListLoad(flag, DivisionName) {
    if (flag == "refresh") {
        ...
        $("#NpdHgmlSubCategory").val("").select2();
    }
...
$("#ExcelDownload").click(function () {
    var data = $('#MyApprovalPending_Grid').jqGrid('getGridParam', 'data');
    if (data.length == 0) {
        ...
        try {
            ...
            $('#MyApprovalPending_Grid').jqGrid("exportToExcel", {
                includeLabels: true,
                includeGroupHeader: true,
                includeFooter: true,
                fileName: "MyApprovalPending.xlsx",
                maxlength: 80,
                exportcol: true,
